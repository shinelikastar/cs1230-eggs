...
VBO::VBO(const float *data, int sizeInFloats, 
		std::vector<VBOAttribMarker> markers, GEOMETRY_LAYOUT layout) :
    m_handle(-1),
    m_markers(markers),
...
void VBO::bindAndEnable() const {
    ...
    for (unsigned int i = 0; i < m_markers.size(); i ++) {
        ...
        glVertexAttribPointer(am.name, am.numElements,
                              am.dataType, am.dataNormalize,
                              m_stride, reinterpret_cast<GLvoid*>(am.offset));
    }
}
